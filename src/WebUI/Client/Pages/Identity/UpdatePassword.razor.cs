...
{
    private readonly UpdatePasswordRequest request = new();
    ...
    [Inject]
    public IValidator<UpdatePasswordRequest> Validator { get; set; } = null!;
    [Inject]
    ...
    public Func<object, string, Task<IEnumerable<string>>> ValidateValue => async (model, propertyName) =>
    {
        var result = await Validator.ValidateAsync(
            ValidationContext<UpdatePasswordRequest>.CreateWithOptions(
    ...
    private async Task SubmitAsync()
    {
        ...
        var response = await Client.UpdatePassword(request);

        if (response.IsSuccessful)
            Snackbar.Add("Password updated", MudBlazor.Severity.Info);
        else
        {
